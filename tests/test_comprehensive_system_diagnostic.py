...
try:
    ...
    try:
        # Test if reasoning traces are properly cleaned (using simple query to avoid timeout)
        test_queries = [
...
try:
    ...
    try:
        ...
        recent = cm.get_context_window(max_turns=5)
        if len(recent) == 5:
    ...
    try:
        ...
        for text, expected_emotion in test_cases:
            ...
            # Use primary_emotion attribute instead of dominant_emotion
            detected = result.primary_emotion if hasattr(result, 'primary_emotion') else result.get("primary_emotion")
    ...
    try:
        ...
        # Add a conversation using correct method
        ...
        # Search for it using correct method
        search_results = sm.semantic_search("programming language", k=1)

        if len(search_results) > 0 and search_results[0].get("similarity", 0) > 0.3:
            log_pass("3.3: Semantic Memory", f"Add & search working (similarity: {search_results[0].get('similarity', 0):.3f})")
        else:
    ...
    try:
        ...
        # Create first instance and add data FIRST
        ...
        # NOW create second instance - it will load the populated store
    ...
    try:
        ...
        if hasattr(pipeline, 'semantic_memory'):
            ...
            log_pass("5.3: Memory Integration",
                    f"Semantic memory loaded: {stats.get('total_conversations', 0)} conversations")
        else:
